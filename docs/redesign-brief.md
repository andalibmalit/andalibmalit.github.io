...
Changes made during the build, after the export (Sept 20), at Andalib's request: the column is centred rather than left-set; body type scales from 19px on phones to 22px on wide screens with a 66ch measure (about 78 characters a line); from 1100px up the section labels hang in the left margin and Research gets a hairline under its title; entry titles are Rasa 600 rather than 500; the theme switch reads "here comes the sun" / "lights out"; the favicon is a ☉ mark (ink ring, ochre dot) that follows the OS colour scheme. Reasons and the CSS-level changes are in "Departures from the handoff" in design/handoff/README.md.
